...
installation. Mod Organizer 2, SkyUI, MCM Helper, and Media Keys Fix SKSE are
not SkyNet requirements.
...
`extern/WebView2`; do not delete that directory. The project build is currently
configured for flat Skyrim SE/AE. Skyrim VR requires its own native UI and input
port and is not supported by this release.
...
```powershell
git clone --recurse-submodules https://github.com/OWNER/SkyNet.git
Set-Location SkyNet
```
...
GitHub repository, then run from this folder:

```powershell
git add .
git commit -m "Initial SkyNet source release"
git remote add origin https://github.com/OWNER/SkyNet.git
git branch -M main
git push -u origin main
```

No project-wide license has been selected. Choose and add one before accepting
outside contributions or publishing derivative-use terms. See
